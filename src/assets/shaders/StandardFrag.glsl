...
{	f = fields_per_light;
	...
	if (hasLighting == 0)
	{	if (solidColor == 1)
		{	objLightingColor = vec4(color, 1);
		}
		else if (envMapped == 1)
		{	vec3 r = -reflect(normalize(-vVertPos), normalize(varyingNormal));
			objLightingColor = vec4((texture(t,r)).xyz, 1);
		}
		else
		{	objLightingColor = vec4((texture(samp,tc)).xyz, 1);
		}
	}
	else // has lighting
	{	if (solidColor == 1)
	...
	if (isTransparent == 1)
	{	if (flipNormal > 0)
		{	fragColor = vec4(objLightingColor.xyz, alpha);
		}
		else
		{	fragColor = vec4(objLightingColor.xyz, alpha*0.8);
		}
	}
	else
	{	fragColor = objLightingColor;
	}
}
