...
//	private HashMap<Component.Identifier, IAction> actions;
	...
	public void update(float time) {
				...
				if (componentState != null) {
					...
					if (event.getComponent().getIdentifier() instanceof Component.Identifier.Axis) {
						...
						if (prettyCloseTo(Math.abs(event.getValue()), 1.0f)) {
							...
							if (componentState.getActionType() == INPUT_ACTION_TYPE.ON_PRESS_AND_RELEASE || componentState.getActionType() == INPUT_ACTION_TYPE.REPEAT_AND_RELEASE) {
		...
		for (Controller c : controllers) {
			...
			for (Component.Identifier component : controllerStates.get(c).keySet()) {
				...
				if (componentState.isNewEvent()) {
					...
					if (componentState.isPressed()
							&& (componentState.getActionType() == INPUT_ACTION_TYPE.REPEAT_WHILE_DOWN || componentState.getActionType() == INPUT_ACTION_TYPE.REPEAT_AND_RELEASE)) {
			...
			if (verbosity > 4) {
				System.out.println("...end InputManager.update() event processing for controller " + c.getName());
			}
		}
