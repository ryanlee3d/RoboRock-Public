...
* <li> set tile factor -- determines the number of tiles along each dimension, if tiling is turned on
...
public class RenderStates
{
	...
	/** sets opacity (alpha) of an object (0.0f to 1.0f), has no effect if isTransparent is false */
	...
	public void setTileFactor(int f) { tileFactor = f; }
	...
	public void setHasSolidColor(boolean sc) { solidColor = sc; }
	...
	public int getTileFactor() { return(tileFactor); }
	...
	* Could be useful if an application wants to bounce back and forth between various render states,
	* and wants to therefore be able to save the current RenderState before temporarily switching to another.
	*/
	public RenderStates makeCopy()
	{	RenderStates temp = new RenderStates();
		if (this.enableRendering) temp.enableRendering(); else temp.disableRendering();
