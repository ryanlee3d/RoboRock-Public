...
public class Camera
{
	...
	/** turns the camera left/right 
	* @param rotation amount in radians (+/- for direction)
	*/
	public void yaw(float radians)
	{
		Vector3f axis = new Vector3f(v).normalize();
		n.rotateAxis(radians, axis.x, axis.y, axis.z);
		u.rotateAxis(radians, axis.x, axis.y, axis.z);
		n.normalize();
		u = (new Vector3f(n)).cross(v).normalize();
		v = (new Vector3f(u)).cross(n).normalize();
	}
	/**
	* Rotates this GameObject around the global/world Y axis (0, 1, 0).
	*
	* @param radians angle to rotate in radians; positive values rotate counterclockwise when viewed from above (assuming +Y is up).
	*/
	public void globalYaw(float radians)
	{
		...
		// Force v to world up (prevents any tilt accumulating)
		v.set(0, 1, 0);
	...
	*/
	...
	{
		Vector3f axis = new Vector3f(u).normalize();
		n.rotateAxis(radians, axis.x, axis.y, axis.z);
		v.rotateAxis(radians, axis.x, axis.y, axis.z);
		n.normalize();
		v = (new Vector3f(u)).cross(n).normalize();
		u = (new Vector3f(n)).cross(v).normalize();
	}
