...
public abstract class VariableFrameRateGame implements KeyListener, MouseListener, MouseMotionListener, MouseWheelListener
{
	...
	{	System.out.println("loading game shapes (models)");
		...
		System.out.println("loading in sounds");
		loadSounds();
	...
	*/
	...
	/**
	* The game application should override this function and create desired initial physics objects.
	* This method is called by the RenderSystem from init(),
	* because the GL context must exist because a shader is used to get heights from a height map
	* (which is needed to build a static mesh physics object for the terrain physics).
	*/
	public void initializePhysicsObjects() { }

	/**
	* The game application should override this function and load all game sounds in it.
	* This method is called once by VariableFrameGame before the game loop starts.
	*/
	...
	*/
	...
	{	System.out.println("starting game loop");
		...
		SceneGraph sg = eng.getSceneGraph();
		sg.updateAllObjectTransforms();
		
		rs.startOpenGLGameLoop();
	...
	* This ensures that the escape key will function to abort a game.
	*/
