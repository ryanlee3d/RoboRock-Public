...
public class GameObject
{
	...
	/** replaces the current RenderStates object with the supplied one */
	public void setRenderStates(RenderStates r) { renderStates = r; }
	...
	*/
	...
	/** rotates the GameObject left/right about its local up axis 
	* @param rotation amount in radians (+/- for direction)
	*/
	public void yaw(float radians)
	{
		Matrix4f oldRot = getLocalRotation();
		Vector3f upAxis = getLocalUpVector().normalize();
		Matrix4f rot = new Matrix4f().rotation(radians, upAxis);
		Matrix4f newRot = new Matrix4f(oldRot).mul(rot);
		setLocalRotation(newRot);
	}
	/**
	 * Rotates this GameObject around the global/world Y axis (0, 1, 0).
	 * Useful for ground-plane turning where yaw should always be about world-up.
	 *
	 * @param radians rotation amount in radians
	 */
	public void globalYaw(float radians)
	{
		Matrix4f oldRot = getLocalRotation();

		// Global/world up axis (do NOT use local up)
		Vector3f worldUp = new Vector3f(0, 1, 0);

		Matrix4f rot = new Matrix4f().rotation(radians, worldUp);
		Matrix4f newRot = new Matrix4f(oldRot).mul(rot);
		setLocalRotation(newRot);
	}
	/** rotates the GameObject up/down about its local right axis 
	* @param rotation amount in radians (+/- for direction)
	*/
	public void pitch(float radians)
	{
		Matrix4f oldRot = getLocalRotation();
		Vector3f rightAxis = getLocalRightVector().normalize();
		Matrix4f rot = new Matrix4f().rotation(radians, rightAxis);
		Matrix4f newRot = new Matrix4f(oldRot).mul(rot);
		setLocalRotation(newRot);
	}
